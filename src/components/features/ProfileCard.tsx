...
export default function ProfileCard({user, isProfileOwner=true}:ProfileCardProps) {
    const router = useRouter();
    console.log("user dans le profile donne", user);
    return (
        <Card className="w-full  mx-auto shadow-lg rounded-xl bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-colors duration-300">
            ...
            <CardContent className="relative p-4">
                ...
                {/* Informations */}
                <h2 className="text-xl font-semibold text-gray-800 dark:text-white">{user?.userName || user?.firstName + " "+ user?.lastName }</h2>
                ...
                <div className="text-gray-700 dark:text-gray-300 mt-2">
                    <div className="flex items-center text-sm gap-1 mb-1">
                        ...
                        <span>Live at <strong>{user?.address}</strong></span>
                    </div>
                    <div className="flex items-center text-sm gap-1">
                        <Calendar className="w-4 h-4 text-green-500" />
                        <span>Membre since <strong>{user?.created}</strong></span>
                    </div>
                </div>
