...
export default function ProfilePage({ params }: ProfilePageProps) {
    ...
    useEffect(() => {
        if (profileId) {
            fetchProfileData(profileId, currentUserId).catch(() => {
                router.push("/404");
            });
            if (!isOwnProfile) {
                getUserById(profileId)
                    ...
                    .catch((error: any) => console.log("[ProfilePage] Erreur récupération:", error));
            }
        } else {
            router.push("/404");
        }
    ...
    return (
        ...
        >
                ...
                <div className="dark:bg-gray-900 p-4 rounded-lg shadow-md flex flex-col">
                    ...
                    <div className="space-y-3">
                        {followers.slice(0, visibleFollowers).map((follower) => (
                            <FriendCard
                                key={follower.follower.idUser}
                                user={follower.follower}
                                isFollowing={following.some(f => f.followed.idUser === follower.follower.idUser)}
                    ...
                    <div className="flex justify-center gap-3 mt-3">
                        {visibleFollowers < followers.length && (
                            <Button variant="ghost" size="icon" onClick={() => setVisibleFollowers(visibleFollowers + 6)}>
                                <FaChevronDown />
                            </Button>
                        )}
                        {visibleFollowers > 6 && (
                            <Button variant="ghost" size="icon" onClick={() => setVisibleFollowers(6)}>
                                <FaChevronUp />
                            </Button>
                        )}
                    </div>
                    ...
                    <div className="dark:bg-gray-900 p-4 rounded-lg shadow-md flex flex-col">
                        ...
                        <div className="flex justify-center gap-3 mt-3">
                            {visibleFollowing < following.length && (
                                <Button variant="ghost" size="icon" onClick={() => setVisibleFollowing(visibleFollowing + 6)}>
                                    <FaChevronDown />
                                </Button>
                            )}
                            {visibleFollowing > 6 && (
                                <Button variant="ghost" size="icon" onClick={() => setVisibleFollowing(6)}>
                                    <FaChevronUp />
                                </Button>
                            )}
                        </div>
            ...
            <main
                className={`flex-1 md:w-1/2 py-6 lg:w-2/3 space-y-6 pb-28 scrollbar-none flex flex-col items-center ${
                    isSingleColumn ? "px-5 pb-18" : "h-[calc(100vh-60px)] overflow-y-auto"
                }`}
            >
                <SubHeader icon={<FaBookmark size={20} />} name={isOwnProfile? "Your Recipe" : "Published Recipe"} sticky={isSingleColumn ? true :false} />
